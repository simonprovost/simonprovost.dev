...
const fragmentShader = `
  ...
  void mainImage(out vec4 O, in vec2 I) {
    ...
    vec2 size = vec2(0.5 * iResolution.x / iResolution.y, 0.5); // Half the size of the canvas in normalized coordinates
    float radius = borderRadius / iResolution.y; // Normalize radius based on resolution
    float sdf = roundedRectSDF(uv, size, radius);
    float alpha = 1.0 - smoothstep(0.0, 0.001, sdf); // Smooth edge for anti-aliasing
...
const GrainyGradient = forwardRef(({ ripples = [], onTimeUpdate }, ref) => {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      rippleTimes: { value: [] },
      rippleCount: { value: 0 },
      borderRadius: { value: 40.0 }, // Border radius in pixels
    }),
    []
  );
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      for (let i = 0; i < Math.min(activeRipples.length, 10); i++) {
        ...
        const normalizedX = (ripple.x / 300) * 2 - 1;
        const normalizedY = 1 - (ripple.y / 200) * 2;
  ...
  return (
    <mesh ref={mesh} scale={[size.width / size.height, 1, 1]}>
